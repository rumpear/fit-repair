...
LAP = (19, [(253, UINT32), (2, UINT32), (3, SINT32), (4, SINT32), (5, SINT32), (6, SINT32),
            (7, UINT32), (8, UINT32), (9, UINT32), (13, UINT16), (14, UINT16)])
...
def straight_ride(n: int, speed: float = 5.0, start: tuple[float, float] = HOME) -> list[Point]:
    """n records one second apart riding due east at a constant speed."""
    return [Point(START + i, *offset(*start, speed * i, 0), dist=speed * i, speed=speed) for i in range(n)]
...
class _Encoder:
    ...
    def define(self, local: int, mesg: tuple[int, list[tuple[int, int]]]) -> None:
        num, fields = mesg
        self.body += struct.pack("<BBBHB", 0x40 | local, 0, 0, num, len(fields))
        for def_num, base in fields:
            self.body += struct.pack("<BBB", def_num, struct.calcsize(_FMT[base]), base)

    def data(self, local: int, mesg: tuple[int, list[tuple[int, int]]], values: list) -> None:
        self.body += bytes([local])
        for (_, base), value in zip(mesg[1], values):
            self.body += struct.pack("<" + _FMT[base], _INVALID[base] if value is None else value)
...
def build(
    ...
    enc.define(3, LAP)
    for a, b in laps or [(0, len(points) - 1)]:
        seg = points[a : b + 1]
        dur = seg[-1].t - seg[0].t
        ...
        enc.data(3, LAP, [fit_time(seg[-1].t), fit_time(seg[0].t), semi(seg[0].lat), semi(seg[0].lon),
                          ...
                          round(max(p.speed for p in seg) * 1000)])
