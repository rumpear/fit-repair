...
def parse_activity(data: bytes) -> Activity:
    ...
    return Activity(data=data, messages=messages, crc_ranges=crc_ranges)
...
def _to_message(frame: fitdecode.FitDataMessage) -> Message:
    ...
    for fd in frame.def_mesg.field_defs:
        ...
        if ref is not None:
            refs[fd.name] = ref
        offset += fd.size
    return Message(name=frame.name, values=values, fields=refs)


def _field_ref(fd, offset: int, endian: str) -> FieldRef | None:
    profile = fd.field
    base = fd.base_type
    if profile is None or profile.subfields or base.name not in INVALID_VALUES:
        return None
    if fd.size != base.size:  # arrays
        return None
